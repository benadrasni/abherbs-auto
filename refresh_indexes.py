"""Local rebuild of Refresher.java indexes.

Reads a catalog dump from disk and writes counts/lists/search/photo JSON.
Does not import firebase_admin and does not write to Realtime Database.
"""
...
def load_catalog(input_dir, stages):
    ...
    if "search" in stages or "photo" in stages:
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--output-dir",
        required=True,
        help="Directory for counts_new.json, lists_new.json, search_new/, search_photo_new.json",
    )
    parser.add_argument(
        "--only",
        choices=STAGES,
